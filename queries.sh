...
echo "$($PSQL "SELECT DISTINCT winner_team.name AS team_name FROM games FULL JOIN teams AS winner_team ON games.winner_id = winner_team.team_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' UNION SELECT DISTINCT opponent_team.name AS team_name FROM games FULL JOIN teams AS opponent_team ON games.opponent_id = opponent_team.team_id WHERE games.year = 2014 AND games.round = 'Eighth-Final';")"
